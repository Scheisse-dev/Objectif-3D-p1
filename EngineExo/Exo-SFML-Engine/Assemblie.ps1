...
function CreateFile
{
    $assembliePath = "$path/Assemblie.h"
    $res = "#pragma once`n"
    $res += "#include `"Engine/Utils/ObjectMacro.h`"`n`n"
    foreach($h in $allDatas)
    {
        $namespace = $h.namespace
        $res += "using namespace $namespace;`n"
        $classes = $h.classes
        for ($i = 0; $i -lt $classes.Count; $i = $i + 1)
        {
            ...
            $res += "#include `"$includePath`"`n"
            $res += "REGISTER_TYPE($className)`n"
        }
        $res += "`n"
    }
    Set-Content -Path $assembliePath -Value $res
}
